...
public class FlutterMetricKitPlugin: NSObject, FlutterPlugin, MXMetricManagerSubscriber,
  FlutterStreamHandler
{
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(
        name: "flutter_metric_kit", binaryMessenger: registrar.messenger)
    let eventChannel = FlutterEventChannel(
        name: "flutter_metric_kit.events", binaryMessenger: registrar.messenger)
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    guard let action = FlutterMetricKitPluginAction.init(rawValue: call.method) else {
      result(
        FlutterError(
          code: "-1", message: "Not implemented", details: "The method is not implemented"))
      return
    }
    switch action {
    case .startReceivingReports:
      if #available(macOS 12.0, *) {
        ...
        result(
          FlutterError(
            code: "-2", message: "Not supported",
            details: "You can only call the method on iOS 13 and above"))
      }
    case .stopReceivingReports:
      if #available(macOS 12.0, *) {
        ...
        result(
          FlutterError(
            code: "-2", message: "Not supported",
            details: "You can only call the method on iOS 14 and above"))
      }
